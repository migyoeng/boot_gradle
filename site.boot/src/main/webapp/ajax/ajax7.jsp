...
<head>
<meta charset="UTF-8">
<title>Jquery - AJAX(POST) + JSON.stringify()</title>
...
$(function(){
	...
	$("#btn").click(function(){
		...
		$.ajax({
			url : "./ajax7.do",	//전송 url 경로
			cache : false,		//캐시 메모리 사용 유무
			type : "POST",		//전송 방식
			dataType : "HTML",	//전송 파일 형태 : HTML, TXT, XML, JSON
			contentType : "application/json; charset=UTF-8",
			data : JSON.stringify({fdata : $ajaxdata}),	//대표키가 fdata로 설정되어 전송
			//data : JSON.stringify($ajaxdata),	//대표키 없이 배열만 전송
			//data : JSON.stringify({$ajaxdata}),	//대표키가 $ajaxdata로 설정되어 전송
				//{"all_data":"한글만"} - 한글 깨짐
				//"all_data=한글만깨짐&all_data2=A형",	//문자열 형태로 전송
				/*
				JSON.stringify({		//배열 형태로 전송			
					name : "홍길동",
					blood : "A형"
				}),
				*/
